...
def process_data(file_path):
    if 'stock' in file_path:
        ...
        destination_cursor.executemany('INSERT INTO stock VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', rows)  
        print("inserted all rows from stock table")
        headers = {"content-type": "text/plain"}
        requests.post(SERVER_URL, data=str(f"DB Updated. {len(rows)} Rows Total"), headers=headers)
    ...
    if 'sales' in file_path:
        ...
        if 'daily' in file_path:
            ...
            print("inserted all rows from stock table")
        headers = {"content-type": "text/plain"}
        requests.post("SERVER_URL", data=str(f"SALES Database Updated. {len(rows)} Rows (15 mins)"), headers=headers)
        ...
        print("inserted all rows from stock table")
        headers = {"content-type": "text/plain"}
        requests.post(SERVER_URL, data=str(f"SALES Database Updated. {len(rows)} Rows (4 hrs)"), headers=headers)
